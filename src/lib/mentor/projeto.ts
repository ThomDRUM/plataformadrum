...
export interface ProjetoOverviewData {
  projectId: string;
  familyName: string;
  overview: { intention: string; mwta: string; point_a: string; point_b: string };
  outcomes: { id: string; text: string }[];
  rules: { id: string; title: string; description: string }[];
  roles: { id: string; person_name: string; description: string }[];
}

export async function getProjetoOverview(mentorId: string): Promise<ProjetoOverviewData | null> {
